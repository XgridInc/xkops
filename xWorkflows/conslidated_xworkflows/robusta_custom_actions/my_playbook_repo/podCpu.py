...
@action
def podCpu(event: PodEvent, params: resizeParams):
    """
    Change the CPU requests of a Pod
    """
    if not event.get_pod():
        # Log an error message if the pod is not found
        logging.error("Failed to get the pod for CPU adjustment")
        return
    ...
    try:
        ...
        logging.info(f"Pod {podName} in namespace {podNamespace} has been updated with new CPU requests.")
    except client.exceptions.ApiException as e:
        logging.error(f"Exception when modifying pod: {e}")


    functionName = "Change CPU requests of Pod"
    finding = Finding(
        title="Pod CPU upgradation",
    ...
    finding.add_enrichment(
        [
            MarkdownBlock(f"{podName} CPU is Upgraded."),
        ]
    )
