...
print_prompt() {
    log "${BBROWN}[INFO]" "[PRE-FLIGHT]" "Initiating pre-flight checks for Kubecost installation to your K8s cluster.${CC}"
}

# Function to check if kubectl is installed in the cluster or not
kubectl_checker() {
    log "${YELLOW}[INFO]" "[PRE-FLIGHT]" "Checking if kubectl is configured.${CC}"
    if command -v kubectl &>/dev/null; then
        log "${BOLD_GREEN}[INFO]" "kubectl configurations are obtained successfully.${CC}"
    else
        log "${BOLD_RED}[ERROR]" "[PRE-FLIGHT]" "kubectl is not configured. Installing now.${CC}"
...
kubectl_installer() {
    ...
    log "${BOLD_GREEN}[INFO]" "[PRE-FLIGHT]" "kubectl is installed successfully.${CC}"
}
