...
print_prompt() {
    log "${BBROWN}[INFO]" "[PRE-FLIGHT]" "Initiating pre-flight checks for Pixie installation to your K8s cluster.${CC}"
}
...
preFlight_checks() {
    ...
    if [[ "${kVers[0]}" -lt 1 && "${kVers[1]}" -le 21 ]]; then
        log "${BOLD_RED}[ERROR]" "[PRE-FLIGHT]" "Unsupported K8s Version: $version. Pixie supports Kubernetes version 1.21+.${CC}"
        exit 1
    fi
    ...
    if [[ "$cpuArch" != "$reqArch" ]]; then
        log "${BOLD_RED}[ERROR]" "[PRE-FLIGHT]" "Unsupported Architecture: $cpuArch.Pixie only supports x86_64 architecture.${CC}"
        exit 1
    fi
    ...
    if [ "$freeMem " -lt 0 ]; then
        log "${BOLD_RED}[ERROR]" "[PRE-FLIGHT]" "Not enough memory on the node. Pixie requires minimum 1 Gib memory per node.${CC}"
        exit 1
    fi
    log "${GREEN}[INFO]" "[PRE-FLIGHT]" "Pre-flights checks passed. Initiating installation...${CC}"
}
