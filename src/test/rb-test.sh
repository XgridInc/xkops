...
check_robusta_actions() {
    node=$(kubectl get nodes -o jsonpath='{.items[0].metadata.name}' > /dev/null )
    results=$(robusta playbooks trigger node_running_pods_enricher name="$node" --namespace robusta > /dev/null )
