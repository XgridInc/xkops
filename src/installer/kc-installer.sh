...
kc_installer() {
    ...
    if [ "$kubecostDeploy" == "kubecost-cost-analyzer" ]; then
        ...
        if ! helm install kubecost kubecost/cost-analyzer -n kubecost --create-namespace &>/dev/null; then
            log "${RED}[ERROR]" "[INSTALLER]" "Failed to install Kubecost. Exiting.${CC}"
            exit 1
        fi

        # Wait for Kubecost Prometheus server pod to be ready
        if ! kubectl -n kubecost wait pod --for=condition=Ready -l component=server --timeout=1h &>/dev/null; then
...
create_kc_service() {
    ...
    if ! kubectl -n kubecost expose deployment kubecost-cost-analyzer --port=80 --target-port=9090 --name=kubecost-ui-service --type=LoadBalancer &>/dev/null; then
