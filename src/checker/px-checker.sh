...
kubectl_pxNS_checker() {     
    ...
    then
        ...
        if [[ "$olm_check" == olm &&  "$pl_check" == pl  &&  "$pxOp_check" == px-operator ]];
        then 
...
kubectl_pxDeploy_checker() {
    ...
    if [[ ${#plDeploy[*]}  -ne 0 && ${#olmDeploy[*]} -ne 0  ]]; then
        ...
        do  
            ns="$PLNS"
            if [[ $dp == "$PL_KELVIN" || $dp == "$PL_CLOUD_CONNECTOR" || $dp == "$PL_VIZIER_QUERY_BROKER" ]];
            then
        ...
        do  
            ns="$OLMNS"
            if [[ $dp == "$OLM_CATALOG_OPERATOR" || $dp == "$OLM_OPERATOR" ]];
            then
                log "${GREEN}[INFO]" "[CHECKER]" "$dp deployment found in $ns namespace.${CC}"
                exit 1
            else 
